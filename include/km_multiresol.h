...
class KM_multiresol {
 public:
  ...
  This function detects core-periphery pairs in the given network. 

 
  Parameters
  ----------
  G : Unipartite network generated by a one-mode projection of bipartite networks
  ...
  */
  ...
  This function returns the quality of the given core-periphery pairs. 
  ...
  q : (Output) q[i] is the quality of the ith core-periphrey pair. We override 'q' instead of returning 'q' as the output of this function in order to reduce the computational cost. 
  ...
  */
  ...
  Getters
  -------
  get_c() : Return the ID of the core-periphery pair to which each node belongs.

  get_x() : Return x, where x[i] = 1 or x[i] = 0 indicates that node i is a core node or a peripheral node, respectively.

  get_q() : Return the quality of each detected core-periphery pair.
...
void KM_multiresol::_label_switching(Graph& G,
  ...
  auto __label_switching = [](Graph& G, vector<int>& c, vector<double>& x, vector<double>& theta,
    ...
    auto _propose_new_label = [](Graph& G, const vector<int>& c, const vector<double>& x, const double th,
      ...
      if (G.contains(node_id) == false) return;
